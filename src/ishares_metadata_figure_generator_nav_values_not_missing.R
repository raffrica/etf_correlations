...
# Usage: Rscript ishares_metadata_figure_generator_nav_values_not_missing.R

library(tidyverse)
library(forcats)

nav_values_remaining <- read_csv("results/ishares_nav_values_not_missing.csv")
...
ggplot(nav_values_remaining, 
  ...
  scale_fill_manual(values = c("no" = "#ABDDDE", 
                               "yes" = "#ECCBAE"),
                    name = "Choice")
ggsave("results/ishares_NAV_3_year.png")
